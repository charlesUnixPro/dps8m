...
typedef struct
  {
    volatile uint port_enable [N_SCU_PORTS];  // enable/disable

    // Mask registers A and B, each with 32 interrupt bits.
    volatile word32 exec_intr_mask [N_ASSIGNMENTS];
    ...
    volatile uint mask_enable [N_ASSIGNMENTS]; // enable/disable
    volatile uint mask_assignment [N_ASSIGNMENTS]; // assigned port number

    volatile uint cells [N_CELL_INTERRUPTS];
    ...
    struct ports {
        ...
        volatile int dev_port [N_SCU_SUBPORTS];
        volatile bool subport_enables [N_SCU_SUBPORTS]; 
        volatile bool xipmask [N_SCU_SUBPORTS]; 
        volatile int xipmaskval;
    } ports[N_SCU_PORTS];
