...
static inline word36 getbits36(word36 x, uint i, uint n) {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36: bad args (%012"PRIu64",i=%d,n=%d)\n", x, i, n);
        return 0;
    } else
        return (x >> (unsigned) shift) & ~ (~0U << n);
}
...
static inline word1 getbits36_1 (word36 x, uint i)
  {
    ...
    int shift = 35-(int)i;
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_1: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word2 getbits36_2 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_2: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word3 getbits36_3 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_3: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word4 getbits36_4 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_4: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word5 getbits36_5 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_5: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word6 getbits36_6 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_6: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word7 getbits36_7 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_7: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word8 getbits36_8 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_8: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word9 getbits36_9 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_9: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word10 getbits36_10 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_10: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word12 getbits36_12 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_12: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word14 getbits36_14 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_14: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word15 getbits36_15 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_15: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word16 getbits36_16 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_16: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word18 getbits36_18 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_18: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word24 getbits36_24 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_24: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word28 getbits36_28 (word36 x, uint i)
  {
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("getbits36_28: bad args (%012"PRIu64",i=%d)\n", x, i);
        return 0;
    } else
...
static inline word36 setbits36(word36 x, uint p, uint n, word36 val)
{
    ...
    if (shift < 0 || shift > 35) {
        sim_printf ("setbits36: bad args (%012"PRIu64",pos=%d,n=%d)\n", x, p, n);
        return 0;
    }
...
static inline word36 setbits36_1 (word36 x, uint p, word1 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("setbits36_1: bad args (%012"PRIu64",pos=%d)\n", x, p);
        return 0;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
...
static inline word36 setbits36_4 (word36 x, uint p, word4 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("setbits36_4: bad args (%012"PRIu64",pos=%d)\n", x, p);
        return 0;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline word36 setbits36_5 (word36 x, uint p, word5 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("setbits36_5: bad args (%012"PRIu64",pos=%d)\n", x, p);
        return 0;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline word36 setbits36_6 (word36 x, uint p, word6 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("setbits36_6: bad args (%012"PRIu64",pos=%d)\n", x, p);
        return 0;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline word36 setbits36_8 (word36 x, uint p, word8 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("setbits36_8: bad args (%012"PRIu64",pos=%d)\n", x, p);
        return 0;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline word36 setbits36_9 (word36 x, uint p, word9 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("setbits36_9: bad args (%012"PRIu64",pos=%d)\n", x, p);
        return 0;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline word36 setbits36_16 (word36 x, uint p, word16 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("setbits36_16: bad args (%012"PRIu64",pos=%d)\n", x, p);
        return 0;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline word36 setbits36_24 (word36 x, uint p, word24 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("setbits36_24: bad args (%012"PRIu64",pos=%d)\n", x, p);
        return 0;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36 (word36 * x, uint p, uint n, word36 val)
  {
    ...
    if (shift < 0 || shift > 35)
      {
        sim_printf ("putbits36: bad args (%012"PRIu64",pos=%d,n=%d)\n", * x, p, n);
        return;
      }
    word36 mask = ~ (~0U << n);  // n low bits on
    mask <<= (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_1 (word36 * x, uint p, word1 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_1: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_2 (word36 * x, uint p, word2 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_2: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_3 (word36 * x, uint p, word3 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_3: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_4 (word36 * x, uint p, word4 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_4: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_5 (word36 * x, uint p, word5 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_5: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_6 (word36 * x, uint p, word6 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_6: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_7 (word36 * x, uint p, word7 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_7: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_8 (word36 * x, uint p, word8 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_8: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_9 (word36 * x, uint p, word9 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_9: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_10 (word36 * x, uint p, word10 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_10: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_12 (word36 * x, uint p, word12 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_12: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_13 (word36 * x, uint p, word13 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_13: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_14 (word36 * x, uint p, word14 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_14: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_15 (word36 * x, uint p, word15 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_15: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_16 (word36 * x, uint p, word16 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_16: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_17 (word36 * x, uint p, word17 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_17: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_18 (word36 * x, uint p, word18 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_18: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_23 (word36 * x, uint p, word23 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_23: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_24 (word36 * x, uint p, word24 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_24: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
...
static inline void putbits36_28 (word36 * x, uint p, word28 val)
{
    ...
    int shift = 36 - (int) p - (int) n;
    if (shift < 0 || shift > 35) {
        sim_printf ("putbits36_28: bad args (%012"PRIu64",pos=%d)\n", *x, p);
        return;
    }
    word36 mask = ~ (~0U<<n);  // n low bits on
    word36 smask = mask << (unsigned) shift;  // shift 1s to proper position; result 0*1{n}0*
