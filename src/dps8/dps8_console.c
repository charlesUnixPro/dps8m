...
static int con_iom3_cmd (int iom_unit_num, UNIT * unitp, pcw_t * pcwp, 
...
t_stat cable_opcon (int iom_unit_num, int chan_num)
  {
    ...
    t_stat rc = cable_to_iom (iom_unit_num, chan_num, 0, DEVT_CON, chan_type_CPI, OPCON_UNIT_NUM, & opcon_dev, & opcon_unit [OPCON_UNIT_NUM], con_iom_cmd, con_iom_io, con_iom3_cmd);
    if (rc)
      return rc;
...
// unless it's not: T&4 puts a command in the PCW.
    ...
    * char_pos = 0;
    ...
    uint tally = lpw . tally;
#if 0
    uint daddr = lpw . daddr;
        ...
        case 023:               // Read ASCII
          {
            ...
            * char_pos = data_dcw . fields . ddcw . cp;
                ...
                if (1)
                  {
                    ...
                    for (int charno = 0; charno < 4; ++charno)
                      {
                        unsigned char c = getbits36(M [daddr], charno * 9, 9);
                        if (c <= 0177 && isprint(c))
