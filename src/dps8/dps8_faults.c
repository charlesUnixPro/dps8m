...
// processor functional cycle. Examples are: APPEND CYCLE, CA CYCLE,
// INSTRUCTION FETCH CYCLE, OPERAND STORE CYCLE, DIVIDE EXECUTION CYCLE. Some
// of these cycles are discussed in various sections of this manual. 
//
...
// without taking any irrevocable action (such as setting PTW.U in an APPEND
// CYCLE or modifying an indirect word in a CA CYCLE), then to discard any
// pending functions (such as an APPEND CYCLE needed during a CA CYCLE), and to
// enter a FAULT CYCLE.
//
...
// to enter a FAULT CYCLE.
//
// Faults in groups 5 or 6 are normally detected during virtual address
// formation and instruction decode. These faults cause the processor to
// suspend overlapped operation, to complete the current and pending
// instructions, and to enter a FAULT CYCLE. If a fault in a higher priority
// group is generated by the execution of the current or pending instructions,
// that higher priority fault will take precedence and the group 5 or 6 fault
// will be lost. If a group 5 or 6 fault is detected during execution of the
// current instruction (e.g., an access violation, out of segment bounds, fault
// during certain interruptible EIS instructions), the instruction is
// considered "complete" upon detection of the fault.
//
...
// instruction as long as no transfer prevents us from returing from the XED
// pair.
// 
// Group 3 faults will probably also execute immediately since a G3 fault
// causes "the processor to "close out" current functions without taking any
// irrevocable action (such as setting PTW.U in an APPEND CYCLE or modifying an
// indirect word in a CA CYCLE), then to discard any pending functions (such as
// an APPEND CYCLE needed during a CA CYCLE), and to enter a FAULT CYCLE."
// 
// Group 4 faults will probably also execute immediately since a G4 fault
// causes "the processor to suspend overlapped operation, to complete current
// and pending functions for the current instruction, and then to enter a FAULT
// CYCLE."
//
// Group 5 & 6 faults will probably also execute immediately because "if a
// group 5 or 6 fault is detected during execution of the current instruction
// (e.g., an access violation, out of segment bounds, fault during certain
// interruptible EIS instructions), the instruction is considered "complete"
// upon detection of the fault." However, remember "If a fault in a higher
// priority group is generated by the execution of the current or pending
// instructions, that higher priority fault will take precedence and the group
// 5 or 6 fault will be lost. If a group 5 or 6 fault is detected during
// execution of the current instruction (e.g., an access violation, out of
// segment bounds, fault during certain interruptible EIS instructions), the
// instruction is considered "complete" upon detection of the fault."
//
...
// 
// Group 7 faults will be processed after next even instruction decode
// instruction decode, but before instruction execution. In this way we can
// actually use bit-28 tp inhibit interrupts
// 
        ...
        cpu.cycle == FAULT_EXEC2_cycle)
      {
        ...
        if (cpu.bTroubleFaultCycle)
        ...
        else
          {
            ...
            if (cpu.cycle == FAULT_EXEC_cycle)
              {
                cpu.bTroubleFaultCycleEven = true;
              }
            else
              {
                cpu.bTroubleFaultCycleEven = false;
              }
          }
        cpu.cycle = FAULT_cycle;
        sim_debug (DBG_CYCLE, & cpu_dev, "Setting cycle to FAULT_cycle\n");
