...
#define OP3 op [2]          // 3rd descriptor (4th ins word)
    ...
    uint    BOLR;           // Boolean result control field
...
#define SF3 SF [2]
    ...
    char    buff [2560];    // a buffer to play with (should be big enough for
                            //  most of our needs)
    ...
    bool    mopSN;          // Sign flag; initially set OFF if the sending
                            ...
                            //  sign is negative.
    bool    mopZ;           // Zero flag; initially set ON and set OFF
                            //  whenever a sending string character that is
                            //  not decimal zero is moved into the receiving
                            //  string.
