...
static uint64 last [N_SCU_UNITS_MAX] = {0, 0, 0, 0};
...
// http://www.multicians.org/chrono.html
        ...
        if (last [scu_unit_num] >= MulticsuSecs)
          {
            sim_debug (DBG_TRACE, & scu_dev, "finagle clock\n");
            MulticsuSecs = last [scu_unit_num] + 1;
          }
        last [scu_unit_num] = MulticsuSecs;
        return last [scu_unit_num];
      }
...
//if_sim_debug (DBG_DEBUG, & scu_dev) {
 //sim_printf ("smic scu_unit_num %o cpu_unit_num %o cpu_port_num %o rega %012llo\n", scu_unit_num, cpu_unit_num, cpu_port_num, rega);
//for (int i = 0; i < 32; i ++) sim_printf (" %o", scu [scu_unit_num] . cells [i]); sim_printf ("\n");
//}
...
t_stat scu_rscr (uint scu_unit_num, uint cpu_unit_num, word18 addr, 
                 word36 * rega, word36 * regq)
  {
    ...
    switch (function)
      {
        case 00: // Read system controller mode register
          {
            // AN-87
            // 0..0 -> A
            // 0..0 -> Q 36-49 (0-13)
...
//sim_printf ("Read system controller mode unit %o %012llo\n", scu_unit_num, * regq);
        ...
        case 01: // Read system controller configuration register
          {
            // AN-87, scr.incl.pl1
            //
            // SCU:
...
//sim_printf ("rscr %d %d%d%d%d%d%d%d%d\n", scu_unit_num, up -> port_enable [0], up -> port_enable [1], up -> port_enable [2], up -> port_enable [3], up -> port_enable [4], up -> port_enable [5], up -> port_enable [6], up -> port_enable [7]);
        ...
        case 04: // Get calendar clock (4MW SCU only)
        case 05: 
          {
#if 0
...
// 12.5 release was 22 years ago
        ...
        case 06: // SU Mode register
        case 07: // SU Mode register
          {
