...
static int eoj (word36 * buffer, uint tally)
  {
    ...
    if (getbits36 (buffer [0], 9, 9) != 014)
      return 0;
    word9 ch = (word9) getbits36 (buffer [0], 18, 9);
    if (ch < '0' || ch > '9')
      return 0;
    ch = (word9) getbits36 (buffer [0], 27, 9);
    if (ch < '0' || ch > '9')
      return 0;
    ch = (word9) getbits36 (buffer [1], 0, 9);
    if (ch < '0' || ch > '9')
      return 0;
    ch = (word9) getbits36 (buffer [1], 9, 9);
    if (ch < '0' || ch > '9')
      return 0;
    ch = (word9) getbits36 (buffer [1], 18, 9);
    if (ch < '0' || ch > '9')
      return 0;
                ...
                for (uint i = 0; i < tally * 4; i ++)
                  {
                    uint wordno = i / 4;
                    uint charno = i % 4;
                    bytes [i] = (uint8) (buffer [wordno] >> ((3 - charno) * 9)) & 0777;
                  }
