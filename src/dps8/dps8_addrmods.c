...
void doComputedAddressContinuation (void)
{
    ...
    //if (modCont->mod)
      //sim_printf("doComputedAddressContinuation(Entry): %d '%s'\n", modCont->mod, modContSTR(modCont));
...
//sim_debug (DBG_TRACE, & cpu_dev, "addrmode rf rpt rd tst %d %d %d %d\n", cu.repeat_first,cu.rpt,cu.rd, (! ((! cu . repeat_first) && (cu . rpt || cu . rd))));
    ...
    IR_MOD_1:
        ...
        if (ISITP(indword) || ISITS(indword))
        {
            if (!doITSITP(iCA, indword, iTAG))
              {
                sim_printf ("ITx in IR_MOD fail!\n");
                return SCPE_UNK;    // some problem with ITS/ITP stuff
              }
        
            if (operType == prepareCA)
            {
//sim_printf ("ITx in IR_MOD: prepareCA; rTAG %o\n", rTAG);
                return SCPE_OK;     // end the indirection chain here
            }
            if (operType == readCY || operType == rmwCY)
            {
//sim_printf ("ITx in IR_MOD: read; rTAG %o\n", rTAG);
...
#endif
// XXX this doesn't make sense, but breaks TestIndirect, TestBugs, and 20184
// XXX if removed
            //return SCPE_OK;
          }
        else
...
//if (foo) 
  //sim_printf ("ITx in IR_MOD; rTAG %o Tm %o\n", rTAG, Tm);
        switch (Tm)
        {
            case TM_IT:
...
//if (foo) sim_printf ("IR_MOD(TM_R): cu.CT_HOLD %o Cr=%06o\n", cu.CT_HOLD, Cr);
...
static void updateIWB (word18 addr, word1 b29, word6 tag)
  {
    cu . IWB = setbits36 (cu . IWB,  0, 18, addr);
    cu . IWB = setbits36 (cu . IWB, 29,  1, b29);
...
t_stat cac (void)
  {
    ...
    // The currentInstrucion . a bit has been qualified with the IGN_B9 bit
    if (i -> a)
      {
        ...
        // Clear the A bit
        updateIWB (i -> address, 0, i -> tag);
        // XXX should the a bit in info be cleared as well?
        // XXX I would rather eliminate that bit...
      }
...
// XXX XXX XXX remember to check directOperandFlag before reading CA
    ...
    RI_MOD:;
        ...
        Read (TPR . CA, & indword, INDIRECT_WORD_FETCH, 0); //TM_RI);
        ...
        if (ISITP (indword) || ISITS (indword))
          {
            if (! doITSITP (TPR . CA, indword, iTAG))
              {
                sim_printf ("IR_MOD_1: doITSITP failed\n");
                return SCPE_UNK;    // some problem with ITS/ITP stuff
              }
          }
        else
...
// XXX Interestingly, this branch doesn't seem to happen in T&D or 20184, at
// least as far as they get.
sim_printf ("need to move character extracting code elsewhere\n");
                // read data where chars/bytes live
                if (operType == readCY || operType == rmwCY) //READOP(i))
                {
                    Read(TPR.CA, &data, OPERAND_READ, i->a);
