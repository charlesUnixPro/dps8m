...
static int allocateSegment (uint seglen, char * segname, uint segno,
  ...
  {
    if_sim_debug (DBG_TRACE, & fxe2_dev)
    ...
    for (int i = 0; i < 5; i ++)
      {
        ...
        int idx = loadSegmentFromFile (pathname);
        if (idx >= 0)
          return idx;
      }
    return -1;
...
t_stat fxe2Dump (UNUSED int32 arg, UNUSED char * buf)
  {
#if 0
    sim_printf ("\nSegment table\n------- -----\n\n");
...
t_stat fxe2 (UNUSED int32 arg, char * buf)
  {
    t_stat run_boot_prep (void);
    if_sim_debug (DBG_TRACE, & fxe2_dev)
      sim_printf ("FXE2: initializing...\n");
...
#endif
        if_sim_debug (DBG_TRACE, & fxe2_dev)
          sim_printf ("Loading segment bound_process_init_\n");
        int segIdx = loadSegment ("bound_process_init_");

        if (segIdx < 0)
          {
            sim_printf ("ERROR: couldn't read bound_process_init_\n");
            return SCPE_OK;
          }
        int nargs = 0;
#if 0
