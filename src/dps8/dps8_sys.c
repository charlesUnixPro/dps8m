...
#ifdef TESTING
static char * lookupSystemBookAddress (word18 segno, word18 offset,
...
t_stat machineRoomPort (UNUSED int32 arg, const char * buf);
t_stat machineRoomPW (UNUSED int32 arg, const char * buf);
...
//
// special dps8 VM commands ....
 
// For greater flexibility, SCP provides some optional interfaces that can be
// used to extend its command input, command processing, and command
// post-processing capabilities. These interfaces are strictly optional and are
// off by default. Using them requires intimate knowledge of how SCP functions
// internally and is not recommended to the novice VM writer.
// 
...
#include "dps8.sha1.txt"
    sim_printf ("%s emulator (git %8.8s)\n", sim_name, COMMIT_ID);
#ifdef TESTING
...
#endif
#ifdef NEED_128
    sim_printf ("#### NEED_128 BUILD ####\n");
#endif
#ifdef WAM
    sim_printf ("#### WAM BUILD ####\n");
#endif
#ifdef HDBG
    sim_printf ("#### HDBG BUILD ####\n");
#endif
#ifdef ROUND_ROBIN
    sim_printf ("#### ROUND_ROBIN BUILD ####\n");
#endif
...
static int addBookComponent (int segnum, char * name, uint txt_start,
...
// Warning: returns ptr to static buffer
static int lookupSystemBookName (char * segname, char * compname, long * segno,
                                 long * offset)
  {
    int i;
    for (i = 0; i < nBookSegments; i ++)
            ...
            for (uint argno = 0; argno < argCount; argno ++)
              {
                ...
                sim_printf ("arg%d value   %05o:%06o[%08o] "
                            "%012"PRIo64" (%"PRIu64")\n", 
...
static t_stat addSystemBookEntry (UNUSED int32 arg, const char * buf)
  {
    ...
    if (addBookComponent (idx, compname, txt_start, txt_len, (int) intstat_start,
                         (int) intstat_length, (int) symbol_start, 
                         (int) symbol_length) < 0)
      return SCPE_ARG;
...
#define bufSz 257
        ...
        if (cnt == 7)
          {
            ...
            if (current >= 0)
              {
                addBookComponent (current, name, txt_start, txt_length,
    ...
    for (int i = 0; i < nBookSegments; i ++)
      { 
        sim_printf ("  %-32s %6o\n", bookSegments[i].segname,
                    bookSegments[i].segno);
        for (int j = 0; j < nBookComponents; j ++)
          {
            if (bookComponents[j].bookSegmentNum == i)
              {
...
static void machineRoomCloseConnection (uv_stream_t* stream);
...
static void machineRoomWriteCallback (uv_write_t * req, int status)
  {
    if (status < 0)
      {
        if (status == -ECONNRESET || status == -ECANCELED ||
          ...
          {
            sim_warn ("machineRoomWriteCallback status %d (%s)\n", -status,
                      strerror (-status));
          }

        // connection reset by peer
        machineRoomCloseConnection (req->handle);
      }
...
//
// machineRoomReadCallback: libuv read complete callback
//
//   Cleanup on read error.
//   Forward data to appropriate handler.

static void machineRoomReadCallback (uv_stream_t* stream,
                             ssize_t nread,
                             const uv_buf_t* buf)
  {
    if (nread < 0)
      {
        if (nread == UV_EOF)
          {
            machineRoomCloseConnection (stream);
          }
      }
    else if (nread > 0)
      {
        telnet_recv (sys_opts.machineRoomTelnetp, buf->base,
                     (size_t) nread);
      }
...
static void machineRoomReadStart (uv_tcp_t * client)
  {
    if (! client || uv_is_closing ((uv_handle_t *) client))
      return;
    uv_read_start ((uv_stream_t *) client, alloc_buffer, machineRoomReadCallback);
  }

// Create and start a write request

static void machineRoomStartWriteActual (uv_tcp_t * client, char * data,
                                        ssize_t datalen)
  {
    if (! client || uv_is_closing ((uv_handle_t *) client))
      return;
...
static void machineRoomStartWrite (uv_tcp_t * client, char * data,
                                 ssize_t datalen)
  {
    if (! client || uv_is_closing ((uv_handle_t *) client))
      return;
    telnet_send (sys_opts.machineRoomTelnetp, data, (size_t) datalen);
  }


static void machineRoomCharForce (UNUSED int conUnitIdx, char ch)
  {
    sim_putchar (ch);
    machineRoomStartWrite (sys_opts.machineRoomClient, & ch, 1);
  }

static void machineRoomPutStrForce (UNUSED int conUnitIdx, char * str)
  {
    size_t l = strlen (str);
    machineRoomStartWrite (sys_opts.machineRoomClient, str, (ssize_t) l);
  }

static void machineRoomStartWriteStr (uv_tcp_t * client, char * data)
  {
    machineRoomStartWrite (client, data, (ssize_t) strlen (data));
  }
...
static void machineRoomConnectPrompt (uv_tcp_t * client)
  {
    machineRoomStartWriteStr (client, "password: \r\n");
    pw_nPos = 0;
  }


static void machineRoomLogon (UNUSED int conUnitIdx, unsigned char * buf, ssize_t nread)
  {
    for (ssize_t nchar = 0; nchar < nread; nchar ++)
      {
        ...
        if (kar == 0x1b || kar == 0x03)             // ESCape ('\e') | ^C
          {
            machineRoomCloseConnection (sys_opts.machineRoomClient);
            return;
          }
#endif
            ...
            switch (kar)
              {
                ...
                case 127:   // delete
                  {
                    ...
                    machineRoomPutStrForce (ASSUME0, "\b \b");
                ...
                case 0x12:  // ^R
                  {
                    machineRoomPutStrForce (ASSUME0, "^R\r\n"); // echo ^R
                    machineRoomConnectPrompt (sys_opts.machineRoomClient);
                    machineRoomPutStrForce (ASSUME0, pw_buffer);
                  }
                 break;
        ...
        if (isprint (kar))   // printable?
          {
            machineRoomCharForce (conUnitIdx, '*');
...
check:;
    ...
    //sim_printf ("<%s>", cpy);
    pw_nPos = 0;
    machineRoomPutStrForce (ASSUME0, "\r\n");

    if (strcmp (cpy, sys_opts.machineRoomPW) == 0)
      {
        machineRoomPutStrForce (ASSUME0, "ok\r\n");
        goto associate;
      }
    else
      {
        //machineRoomPutStrForce ("<");
        //machineRoomPutStrForce (pw_buffer);
        //machineRoomPutStrForce (">\r\n");
        machineRoomPutStrForce (ASSUME0, "nope\r\n");
        goto reprompt;
      }
...
t_stat machineRoomPort (UNUSED int32 arg, const char * buf)
  {
    ...
    if (n < 0 || n > 65535) // 0 is 'disable'
      return SCPE_ARG;
    sys_opts.machineRoomPort = n;
    sim_printf ("Machine room port set to %d\n", n);
    return SCPE_OK;
  }

t_stat machineRoomPW (UNUSED int32 arg, UNUSED const char * buf)
  {
    if (strlen (buf) == 0)
      {
        sim_printf ("no password\n");
        sys_opts.machineRoomPW[0] = 0;
        return SCPE_OK;
      }
    ...
    if (strlen (token) > PW_SIZE)
      return SCPE_ARG;
    strcpy (sys_opts.machineRoomPW, token);
    //sim_printf ("<%s>\n", token);
    return SCPE_OK;
  }

static void machineRoomCloseCallback (uv_handle_t * stream)
  {
    free (stream);
...
static void machineRoomCloseConnection (uv_stream_t* stream)
  {
    sim_printf ("Machine room disconnect\n");
    // Clean up allocated data
    if (sys_opts.machineRoomTelnetp)
      {
        telnet_free (sys_opts.machineRoomTelnetp);
        sys_opts.machineRoomTelnetp = NULL;
      }
    if (! uv_is_closing ((uv_handle_t *) stream))
      uv_close ((uv_handle_t *) stream, machineRoomCloseCallback);
    sys_opts.machineRoomClient = NULL;
  }

static void processMachineRoomInput (UNUSED int conUnitIdx, unsigned char * buf,
                                 ssize_t nread)
  {
    if (sys_opts.machineRoomInBuffer)
      {
        //sim_warn ("machhineRoomInBuffer overrun\n");
        unsigned char * new =
          realloc (sys_opts.machineRoomInBuffer,
                   (unsigned long) (sys_opts.machineRoomInSize + nread));
        if (! new)
          {
            sim_warn ("machineRoomInBuffer realloc fail; dropping data\n");
            goto done;
          }
        memcpy (new + sys_opts.machineRoomInSize, buf, (unsigned long) nread);
        sys_opts.machineRoomInSize += nread;
        sys_opts.machineRoomInBuffer = new;
      }
    else
      {
        sys_opts.machineRoomInBuffer = malloc ((unsigned long) nread);
        if (! sys_opts.machineRoomInBuffer)
          {
            sim_warn ("machineRoomInBuffer malloc fail;  dropping data\n");
            goto done;
          }
        memcpy (sys_opts.machineRoomInBuffer, buf, (unsigned long) nread);
        sys_opts.machineRoomInSize = (uint) nread;
        sys_opts.machineRoomInUsed = 0;
      }
...
static void machineRoomTelnetReadCallback (UNUSED uv_tcp_t * client,
                            ssize_t nread,
                            unsigned char * buf)
  {
    if (sys_opts.loggedOn)
      processMachineRoomInput (0, buf, nread);
    else
      machineRoomLogon (0, buf, nread);
  }
    ...
    switch (event->type)
      {
        case TELNET_EV_DATA:
          {
            machineRoomTelnetReadCallback (client, (ssize_t) event->data.size,
        ...
        case TELNET_EV_SEND:
          {
            machineRoomStartWriteActual (client, (char *) event->data.buffer,
        ...
        case TELNET_EV_IAC:
          {
            if (event->iac.cmd == 243 || // BRK
              ...
              {
                sim_warn ("libtelnet dropping unassociated machine room BRK/IP\n");
              }
            else
...
static void * machineRoomTelnetConnect (uv_tcp_t * client)
  {
...
//
// Connection callback handler for machine room
//

static void onNewMachineRoom (uv_stream_t * server, int status)
  {
    if (status < 0)
      {
    ...
    uv_tcp_init (sys_opts.machineRoomLoop, client);
    client->data = server->data;
    if (uv_accept (server, (uv_stream_t *) client) == 0)
      {
        // Only a single connection at a time
        if (sys_opts.machineRoomClient)
          {
            sim_printf ("machine room client cutting in\r\n");
            sys_opts.loggedOn = false;
            machineRoomCloseConnection ((uv_stream_t *) sys_opts.machineRoomClient);
          }
        sys_opts.machineRoomClient = client;
        struct sockaddr name;
        int namelen = sizeof (name);
        int ret = uv_tcp_getpeername (sys_opts.machineRoomClient, & name, & namelen);
        if (ret < 0)
          {
            sim_printf ("Machine room connect;addr err %d\n", ret);
          }
        else
          {
            struct sockaddr_in * p = (struct sockaddr_in *) & name;
            sim_printf ("Machine room connect %s\n", inet_ntoa (p->sin_addr));
          }


        sys_opts.machineRoomTelnetp = machineRoomTelnetConnect (sys_opts.machineRoomClient);
        ...
        sys_opts.loggedOn =
          ! strlen (sys_opts.machineRoomPW);
        if (! sys_opts.loggedOn)
          machineRoomConnectPrompt (sys_opts.machineRoomClient);
        machineRoomReadStart (sys_opts.machineRoomClient);
      }
    else
      {
        uv_close ((uv_handle_t *) client, machineRoomCloseCallback);
      }
  }

static void uv_open_machineRoom (int conUnitIdx, int portno)
  {
    if (! portno)
      {
        //sim_printf ("machine room port disable\n");
        return;
      }
    if (! sys_opts.machineRoomLoop)
      sys_opts.machineRoomLoop = uv_default_loop ();

    //sim_printf ("uv_open_machineRoom %d\n", portno);

    // Do we already have a listening port (ie not first time)?
    if (sys_opts.machineRoomOpen)
      return;

    uv_tcp_init (sys_opts.machineRoomLoop, & sys_opts.machineRoomServer);
    //csp->console_server.data = (void *) (uintptr_t) conUnitIdx;
    struct sockaddr_in addr;
    uv_ip4_addr ("0.0.0.0", portno, & addr);
    uv_tcp_bind (& sys_opts.machineRoomServer, (const struct sockaddr *) & addr, 0);
//sim_printf ("machine room listening on port %d\n", portno);
#define DEFAULT_BACKLOG 16
    int r = uv_listen ((uv_stream_t *) & sys_opts.machineRoomServer,
                       DEFAULT_BACKLOG, 
                       onNewMachineRoom);
    if (r)
     {
    ...
    sys_opts.machineRoomOpen = true;

  }

void startMachineRoom(void)
  {
    sys_opts.machineRoomInBuffer = NULL;
    sys_opts.machineRoomInSize = 0;
    sys_opts.machineRoomInUsed = 0;
    uv_open_machineRoom (ASSUME0, sys_opts.machineRoomPort);
  }
