...
static int mt_iom3_cmd (int iom_unit_num, UNIT * unitp, pcw_t * pcwp, 
...
t_stat cable_mt (int mt_unit_num, int iom_unit_num, int chan_num, int dev_code)
  {
    ...
    t_stat rc = cable_to_iom (iom_unit_num, chan_num, dev_code, DEVT_TAPE, chan_type_PSI, mt_unit_num, & tape_dev, & mt_unit [mt_unit_num], mt_iom_cmd, mt_iom_io, mt_iom3_cmd);
    if (rc)
      return rc;
...
static int mt_iom3_cmd (int iom_unit_num, UNIT * unitp, pcw_t * pcwp, 
                        ...
                        word1 * is_read)
  { 
// The tape channel is a PSI; ignore the PCW; do the payload channel mbx
// commands.
    ...
    * stati = 0;
    ...
    sim_debug (DBG_DEBUG, & iom_dev,
               ...
               __func__, mbx_addr);
    lpw_t lpw;
    fetch_and_parse_lpw (& lpw, mbx_addr, false);
    sim_debug (DBG_DEBUG, & iom_dev,
               "LPW at %#06o: %s\n", mbx_addr,
               lpw2text (& lpw, false));
    sim_debug (DBG_TRACE, & iom_dev,
               "Payload LPW at %#06o: %s\n", 
                mbx_addr, lpw2text (& lpw, true));

#if 0 // XXX
    if (lpw . nc != 0 || lpw . trunout != 1)
      {
        sim_printf ("ERROR: mt_iom3_cmd can't hack tally control != 01 %u%d\n",
                    lpw . nc, lpw . trunout);
        return 1;
      }
    ...
    if (lpw . ae)
      {
        sim_printf ("ERROR: mt_iom3_cmd can't hack ae != 0\n");
        return 1;
      }
    ...
    // We expect a command (IDCW), or a command and a parameter (IDCW, DDCW).

    if (dcw . type != idcw)
      {
        sim_printf ("ERROR: mt_iom3_cmd can't hack type != idcw\n");
        return 1;
      }

    if (lpw . ires)
      {
        sim_printf ("ERROR: mt_iom3_cmd can't hack lp2 . ires\n");
        return 1;
      }
    ...
    switch (dcw . fields . instr . dev_cmd)
      {
#if 0
        ...
        case 5: // CMD 05 -- Read Binary Record
          {
            ...
            uint ddcw_addr = lpw . dcw_ptr;
            sim_debug (DBG_DEBUG, & iom_dev, "%s: DCW @ 0%06o\n", 
                       __func__, ddcw_addr);

            dcw_t data_dcw;
            // XXX check 1 bit; read_only
            fetch_and_parse_dcw (iom_unit_num, chan, & data_dcw, ddcw_addr, 1);
            sim_debug (DBG_INFO, & iom_dev, "%s: DDCW is: %s\n",
                       __func__, dcw2text (& data_dcw));
            sim_debug (DBG_TRACE, & iom_dev, "DDCW is: %s\n",
                       dcw2text (& data_dcw));

            if (data_dcw . type != ddcw)
              {
                sim_printf ("ERROR: mt_iom3_cmd can't hack type != ddcw\n");
                return 1;
              }

            uint type = data_dcw . fields .  ddcw . type;
            uint daddr = data_dcw . fields . ddcw . daddr;
            uint tally = data_dcw . fields . ddcw . tally;   // FIXME?
            uint cp = data_dcw . fields . ddcw . cp;

            if (type == 3)
              {
                sim_printf ("can't hack type == 3\n");
                return 1;
              }
            if (tally == 0)
              {
        ...
        default:
          {
            * stati = 04501;
            sim_debug (DBG_ERR, & tape_dev,
                       "%s: Unknown command 0%o\n", __func__, dcw . fields . instr . dev_cmd);
          }
          break;
      }
// XXX
//sim_printf ("verify tally runout on channel commands.\n");
    return 0;
  }
