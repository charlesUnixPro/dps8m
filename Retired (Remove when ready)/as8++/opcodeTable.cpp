...
opCode allOpcodes[] =
{
	...
	{"callsp", OPCODE, 0, 0, false, 0713, 0713000},
...
mapInstab *InstructionTable = new mapInstab();
...
initInstructionTable()
{
    ...
    for(int i = 0 ; i < sizeof(allOpcodes) / sizeof(opCode); i += 1, src += 1)
    {
        if (src->mne)
        {
            ...
            InstructionTable->insert(std::pair<char const *, opCode *>(src->mne, src));
     
            nCount += 1;
         }
    }
    if (debug)
        fprintf(stderr, "%d opcodes added to instrctuon table ...\n", nCount);
}
