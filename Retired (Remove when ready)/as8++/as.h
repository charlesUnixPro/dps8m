...
// string comparator functions for various lists/maps .....
...
typedef std::map<char const *, opCode *, ltcasestr> mapInstab;
extern mapInstab *InstructionTable;

void initInstructionTable();
opCode *findOpcode(char const *op);

word18 getOpcode(char const *op);   ///< , const char *arg1, const char *arg2);
opCode *getOpcodeEntry(char const *op);
word18 getEncoding(char const *op);    /// , const char *arg1, const char *arg2) {
...
typedef std::map<char const *, symtab *, ltstr> mapSymtab;
extern mapSymtab *SymbolTable;
...
extern literal literalPool, *litPool;
extern int seqNo;
...
struct pseudoOp
{
    const char *name;     ///< name of pseudo op
    pFlags flags;   ///< flags
...
outRec *outas8(word36 data, word18 address, char const *srctext);
outRec *outas8Str(char *str, word18 address, char const *srctext);    // data as 12-digit octal string
...
void doLongCall(char *, word18);                // multics CSR Call pseudo-op
