...
void doBoolEqu(char *sym, word36 val)
{
    symtab *s = getsym(sym);
...
{    
    ...
    for (mapSymtab::iterator it = SymbolTable->begin(); it != SymbolTable->end(); ++it)
	{
        ...
        if (s->segname)
        {
            sprintf(temp, "%s %s %06llo", s->segname, s->name, s->value & AMASK);
...
void doPush(word36 size)                       // multics CSR Push Pseudo-op
{
    ...
    sprintf(w, "%06o%06o", (word18)((size + 060) & AMASK), getEncoding("eax7"));
    outas8Str(w, addr, LEXline);
    addr += 1;
...
void doLongCall(char *entrypoint, word18 arglist)               // multics CSR Call pseudo-op
{
    if (nPass == 1)
    {
        addr += 7;
    ...
    sprintf(w, "%o%05o%06o", 6, 0,  getEncoding("spri") | (word18)BIT(29));
    outas8Str(w, addr, LEXline);
    addr += 1;

    sprintf(w, "%o%05o%06o", 6, 32,  getEncoding("sreg") | (word18)BIT(29));
    outas8Str(w, addr, NULL);
    addr += 1;

    sprintf(w, "%06o%06o", arglist & AMASK,  getEncoding("epp0"));
    outas8Str(w, addr, NULL);
    addr += 1;
    ...
    sprintf(w, "%o%05o%06o", 6, 0, getEncoding("lpri") | (word18)BIT(29));
    outas8Str(w, addr, NULL);
    addr += 1;
    
    sprintf(w, "%o%05o%06o", 6, 32, getEncoding("lreg") | (word18)BIT(29));
    outas8Str(w, addr, NULL);
    addr += 1;
...
void doTemp(pseudoOp *p, tuple *t)                // for TEMP pseudo-ops
{
...
pseudoOp pseudoOps[] =
{
    {"bss",         epUnknown,      NULL,      BSS },
    {"tally",       epUnknown,      NULL,    TALLY },
    {"tallyb",      epUnknown,      NULL,    TALLY },
    {"tallyc",      epUnknown,      NULL,    TALLY },
    {"tallyd",      epUnknown,      NULL,    TALLY },
    {"dec",         epDEC,          NULL,      DEC },
    {"oct",         epOCT,          NULL,      OCT },
    {"arg",         epUnknown,      NULL,      ARG },
    {"zero",        epUnknown,      NULL,     ZERO },
    {"vfd",         epVFD,          NULL,      VFD },
    {"even",        epUnknown,      NULL, PSEUDOOP },
    {"odd",         epUnknown,      NULL, PSEUDOOP },
    {"eight",       epUnknown,      NULL, PSEUDOOP },
    {"sixtyfour",   epUnknown,      NULL, PSEUDOOP },
    {"org",         epUnknown,      NULL,      ORG },
...
//  {"include",     0, doInclude },
    {"its",         epUnknown,      NULL,      ITS },
    {"itp",         epUnknown,      NULL,      ITP },
    {"null",        epUnknown,      NULL,   NULLOP },
    {"rem",         epUnknown,      NULL,   NULLOP },
    {"mod",         epUnknown,      NULL,      MOD },
    
    
    // Honeywell/GE subroutine call/save/return
    {"call",        epUnknown,      NULL,     CALL },
    {"save",        epUnknown,      NULL,     SAVE },
    {"return",      epUnknown,      NULL,   RETURN },
...
//  {"erlk",        0, doErlk    },
    ...
    {"ndsc9",       epUnknown,      NULL,    DESC2 },
    {"ndsc4",       epUnknown,      NULL,    DESC2 },
    {"bdsc",        epUnknown,      NULL,    DESC2 },
...
//    {"sbdx",        0, doARS    },  //Subtract  bit Displacement from Address Register
    ...
    {"segment",     epUnknown, doSegment },
    {"go",          epUnknown, doGo      },
    
    {"name",        epUnknown, doName,   NAME  },     ///< segment name directive
    {"segdef",      epUnknown, NULL,    SEGDEF},     ///< segdef directive
    {"segref",      epUnknown, NULL,    SEGREF},
    
    { NULL, epUnknown, 0 } ///< end marker do not remove
};
