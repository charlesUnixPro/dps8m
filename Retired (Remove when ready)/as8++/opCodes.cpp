...
opCode *findOpcode(char const *op)
{
    ...
    mapInstab::iterator it = InstructionTable->find(op);
...
{
    outRec *p = (outRec*)calloc(1, sizeof(outRec));  // calloc() zeros-out memory
...
outRec *outas8(word36 data, word18 address, char const *srctext)
{
    ...
    p->data = data;
    p->src = srctext ? strdup(srctext) : (char*)"";
...
outRec *outas8Str(char *s36, word18 address, char const *srctext)
{
