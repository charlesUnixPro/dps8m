 ...
 */
expr *findExtRef(tuple *t)
{
    symtab *s = Symtab;
    while (s)
    {
        if (s->Value->type == eExprLink)
            if (!strcmp(s->segname, t->a.p) && !strcmp(s->extname, t->b.p))
                return s->Value;
...
void doLink(char *s, tuple *t)
{
    if (nPass == 1)
    {
        ...
        else
        {
            ...
            //XXXX cant update linkCount if a ext ref already exists
            //sym = addsym(a, 0);
            sym = addsymx(s, e);
